...
/*   Updated: 2023/01/22 22:06:10 by rlarabi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_game	read_map(int fd)
{
	...
	while (1)
	{
		...
		if (!ft_strncmp(temp, "\n", 2))
			m.error = -1;
		ret = m.origin_map;
		m.origin_map = ft_strjoin(ret, temp);
		m.height++;
		free(ret);
		free(temp);
	}
	m.map = ft_split(m.origin_map, '\n');
...
int	check_path(char **map, int a, int b)
{
	...
	count = 0;
	if (map[a][b] == '1' || map[a][b] == 'E' || map[a][b] == 'x'
		|| map[a][b] == 'I')
		return (count);
