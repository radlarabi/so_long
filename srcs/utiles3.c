...
/*   Updated: 2023/01/22 22:12:49 by rlarabi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_map_1(t_game m)
{
	...
	if (!if_exist(m.map, 'E') || !if_exist(m.map, '1') || !if_exist(m.map, 'C')
		|| !if_exist(m.map, 'P') || !is_duplicate(m.map, 'P')
		|| !is_duplicate(m.map, 'E'))
	{
...
void	display_map_in_window(t_image_data *i_data)
{
	...
	map = ft_split(i_data->game.origin_map, '\n');
	a = -1;
	while (map[++a])
	{
		b = -1;
